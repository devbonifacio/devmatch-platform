...
export default function Discover() {
  ...
  const [friendStatuses, setFriendStatuses] = useState({}); // userId → 'sent'|'friend'
  ...
  return (
    <div className="min-h-screen px-6 py-10 animate-page-enter">
      <div className="mx-auto max-w-lg">
        ...
        >
          {toast && (
            <div className={`rounded-2xl px-5 py-3 text-sm font-semibold shadow-lg backdrop-blur-sm ${
              toast.type === "like" ? "bg-brand-500 text-white" : "bg-dark-700 text-slate-300"
            }`}>
              {toast.message}
            </div>
          )}
        ...
        <div
          ...
          style={{
            ...
            opacity: showKeyHint && !isSearchMode ? 1 : 0,
            transform: showKeyHint && !isSearchMode ? "translateX(-50%) translateY(0)" : "translateX(-50%) translateY(8px)",
          }}
        >
              ...
              <div className="space-y-3">
                {searchResults.map((u) => (
                  <SearchResultCard
                    key={u._id}
                    user={u}
                    status={friendStatuses[u._id]}
                    onFriendRequest={() => handleFriendRequest(u._id)}
                  />
                ))}
              </div>
            )}
          ...
          <>
            {loading ? (
              ...
              <div className="relative">
                {nextDev && (
                  <div className="card absolute inset-x-5 top-4 opacity-30" style={{ zIndex: 0, height: "80px" }} />
                )}
                {developers[2] && (
                  <div className="card absolute inset-x-8 top-2 opacity-15" style={{ zIndex: 0, height: "60px" }} />
                )}
  ...
  return (
    <div className="card flex items-center gap-4 p-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        {user.stack?.length > 0 && (
          <p className="mt-0.5 truncate text-xs text-slate-500">{user.stack.slice(0, 4).join(" · ")}</p>
        )}
        {user.bio && <p className="mt-0.5 truncate text-xs text-slate-600">{user.bio}</p>}
      </div>
      {status === "sent" ? (
        <span className="flex-shrink-0 rounded-xl px-3 py-1.5 text-xs font-semibold text-green-400 bg-green-500/10">
          Enviado
        </span>
      ) : status === "friend" ? (
        <span className="flex-shrink-0 rounded-xl px-3 py-1.5 text-xs font-semibold text-brand-400 bg-brand-500/10">
          Amigo
        </span>
      ) : (
        <button
          onClick={onFriendRequest}
          className="flex-shrink-0 flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold text-white transition-all active:scale-95"
